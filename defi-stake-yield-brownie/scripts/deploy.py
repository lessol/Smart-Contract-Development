...
def deploy_token_farm_and_dapp_token():
    ...
    # send out all or most of the dappToken to the TokenFarm contract to give out as reward
    tx = dapp_token.transfer(token_farm.address, dapp_token.totalSupply() - KEPT_BALANCE, {"from":account})
    tx.wait(1)
    # 3 allowed tokens to be staked on tokenFarm:  dapp_token, weth_token, fau_token/dai
    weth_token = get_contract("weth_token")
    fau_token = get_contract("fau_token")
    dict_of_allowed_tokens = {
        dapp_token: get_contract("dai_usd_price_feed"), # dappToken = dai
        fau_token: get_contract("dai_usd_price_feed"), # fauToken = dai
